...
export const addUser = async (userData) => {
  ...
  const client = await pool.connect(); // 1. Get a client for transaction
  ...
  try {
    ...
    // C. AUTOMATICALLY INSERT 5 SICK & 5 VACATION LEAVES
    // This query finds the IDs for 'Sick Leave' and 'Vacation Leave' 
    // and inserts 5 days for the current year.
    await client.query(
    ...
    await client.query("COMMIT"); // 3. Commit changes
    return newUser;

  } catch (error) {
    await client.query("ROLLBACK"); // 4. Rollback if error
    throw error;
  } finally {
    client.release(); // 5. Release client
  }
};
